...
def build_choropleth_hideout(color_prop):
	hideout = dict(colorscale=fundraising_colorscale, classes=fundraising_classes, style=fundraising_style, 
...
def get_side_panel_form(candidates, df):
    return html.Div(children=[
			plotting.create_candidate_funds_bar_plot(candidates, df),
			dbc.Collapse(
				children=[],
				id="candidate_info_collapse"
			)
		],
        id="side-panel-form",
...
def get_sidebar_layout(db):
	...
	return dbc.Container([
		dbc.Row([
			dbc.Col(
				get_side_panel_layout(candidates, mec_df),
				width=4
			),
			dbc.Col(
