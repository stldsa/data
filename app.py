...
def process_election_data_row(row):
    ...
    return dict(
        ward=cells[0],
        stats=dict(reg_voters=int(cells[1]), ballots_cast=int(cells[2])),
        president=dict(trump=int(cells[3]), biden=int(cells[4])),
        prop_d=dict(yes=int(cells[5]), no=int(cells[6])),
        prop_1=dict(yes=int(cells[7]), no=int(cells[8])),
        prop_r=dict(yes=int(cells[9]), no=int(cells[10])),
    )
...
with open(ward_geojson_path) as read_file:
    ...
    for ward_object in ward_geojson["features"]:
        ...
        ward_object["properties"]["extra_data"] = results_by_ward[
            str(int(ward_object["properties"]["Ward"]))
        ]
...
city_map = html.Div(
    children=[
        dl.Map(
            dl.LayersControl([base_tile_layer, ward_overlay]),
            zoom=11,
            center=[38.681, -90.313],
            style={
                "width": "100%",
                "height": "100vh",
                "margin": "none",
