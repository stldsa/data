...
for election_date in election_dates:
	election_options.append({'label':election_date_names[election_date], 'value':election_date})
election_dropdown = dcc.Dropdown(id='election-dropdown', options=election_options, searchable=False, placeholder="Select an election")
...
side_panel_form = html.Div(
	...
	id='side-panel-form',
	style={"lineHeight":"1.4em","width":"90%","padding":"10px","barderRight":"4px solid red"}
)

# Info Panel - This shows the info of the clicked/selected element
info_panel = html.Div([
	html.Div([], id="ward-info-panel"),
	html.Div([], id="precinct-info-panel")
], style={"width":"90%", "height":"70%"})

side_panel_style={"width": "30%", "height": "100vh", "maxWidth":"500px",
...
@app.callback(Output("ward-info-panel", "children"), [Input("wards-geojson", "click_feature")], State('election-dropdown', 'value'))
def ward_click(feature, election_selected):
	div_children = []
	races_per_election = dict(
		aug_2020=dict(
			rep1_dem=dict(name="U.S Congress MO-1 (Democratic Primary)", candidates=["Clay","Bush"]),
			ag_dem=dict(name="MO Attorney General (Democratic Primary)", candidates=["Finneran","Gross"]),
			treas_dem=dict(name="STL City Treasurer (Demorcatic Primary)", candidates=["Jones","Boyd"])
		),
		nov_2020=dict(
			president=dict(name="President", candidates=["Trump", "Biden"]),
			prop_1=dict(name="Prop 1 (Residency Repeal)", candidates=["Prop 1 Yes", "Prop 1 No"]),
			prop_d=dict(name="Prop D (Approval Voting)", candidates=["Prop D Yes", "Prop D No"]),
			prop_r=dict(name="Prop R (Education Tax)", candidates=["Prop R Yes", "Prop R No"])
		))
	...
	if feature is not None:
		...
		feature_name_style = {"textAlign":"center", "fontSize":"1.6em", "fontWeight":"bold", "textDecoration":"underline"}
		...
		if election_selected is not None:
			...
			div_children.append(html.Div("Turnout: {0:.2f}%".format(turnout_percentage), style={"textAlign":"center"}))
			...
			for i in range(len(elec_races)):
				...
				split_text = '<br>'.join(textwrap.wrap(race_names[elec_races[i]], width=21)) # neat little solution for wrapping titles: https://community.plotly.com/t/wrap-long-text-in-title-in-dash/11419
				subplot_titles.append(split_text)
			elec_donuts = make_subplots(rows=len(elec_races), cols=1, specs=subplot_specs, subplot_titles=subplot_titles)
			...
			for race in elec_races:
				...
				race_legendgroup = 'a'
				race_pie_layout = go.Layout(title=race_name, showlegend=True, autosize=True)
				race_pie_labels = []
				race_pie_counts = []
				...
				i=i+1
				elec_donuts.append_trace(go.Pie(labels=race_pie_labels, legendgroup=race_legendgroup, values=race_pie_counts, name=race_name, hole=0.35), i, 1)
			elec_donuts.update_traces(hoverinfo='label+percent', textinfo='none', textfont_size=15, marker={"line":{"color":"#000000", "width":2}})
			...
			ward_election_results_style={"height":"100%","maxHeight":"380px","overflowY":"scroll","border":"1px solid black"}
			div_children.append(html.Div(ward_election_results, style=ward_election_results_style))
		else:
			...
			stats_container = html.Div(stats_table, style={"overflowY":"scroll", "maxHeight":"200px", "border":"1px solid"})
			div_children.append(html.Br())
			div_children.append(stats_container)
	else: 	
		# Ward info header
		ward_info_header_style = {"border":"2px dashed","padding":"8px"}
		div_children.append(html.Div(children=[
			html.Em("(Choose an election and click on a ward to see info about that ward)")
	...
	trace3 = go.Bar(
		x=['2000 Population', '2010 Population'],
		y=[
			extra_data['total_pop_2000']-extra_data['black_pop_2000']-extra_data['white_pop_2000'],
			extra_data['total_pop_2010']-extra_data['black_pop_2010']-extra_data['white_pop_2010']
		],
		name='Other'
	)
	pop_graph_layout = go.Layout(title="Ward population: 2000 -> 2010",
		barmode='stack', showlegend=False, height=300)
	pop_graph = dcc.Graph(id='population_graph', figure=go.Figure(data=[trace1, trace2, trace3], layout=pop_graph_layout))
	pop_bar_graph_section = html.Div(children=[pop_graph])

	# Housing pie graphs
	labels = ['Occupied','Vacant']
	housing_unit_data_2010 = [extra_data['occupied_housing_units_2010'], extra_data['vacant_housing_units_2010']]
	housing_unit_data_2000 = [extra_data['occupied_housing_units_2000'], extra_data['vacant_housing_units_2000']]
	
	housing_pie_layout = go.Layout(title = "Housing Units", 
		showlegend = False, autosize=True)
	housing_pie_2010 = dcc.Graph(id='housing_pie_2010',
		figure=go.Figure(data=[go.Pie(labels=labels, values=housing_unit_data_2010)],
			layout=housing_pie_layout))
	housing_pie_2000 = dcc.Graph(id='housing_pie_2000',
		figure=go.Figure(data=[go.Pie(labels=labels, values=housing_unit_data_2000)],
			layout=housing_pie_layout))
...
with open(precinct_geojson_path) as read_file:
	...
	for precinct_object in precinct_geojson["features"]:
		df_precinct = df.loc[df['HANDLE'] == int(precinct_object["properties"]["HANDLE"])]
		precinct_object["properties"]["extra_data"] = dict(est_white=int(df_precinct.values[0][3]), est_black=int(df_precinct.values[0][4]))
...
city_map = html.Div(
	children=[
		dl.Map(
			dl.LayersControl([base_tile_layer, ward_overlay, precinct_overlay, neighborhood_overlay]),
			zoom=12, center=[38.648, -90.253], 
